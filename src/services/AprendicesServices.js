...
export const crearAprendiz = async (form) => {
  const res = await axios.post(API_BASE, form, { headers: { "Content-Type": "application/json" } });
  return res.data;
};

export const actualizarAprendiz = async (id, form) => {
  const res = await axios.put(`${API_BASE}/${id}`, form, { headers: { "Content-Type": "application/json" } });
  return res.data;
};
